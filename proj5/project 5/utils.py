...
def projectImage(frames, sourceFrameIndex, referenceFrameIndex,
    ...
    projectedImage = cv2.warpPerspective((frames[sourceFrameIndex]*255).astype(np.uint8),
                                         H_t.dot(pastHomographies[sourceFrameIndex,
                                         referenceFrameIndex]), (xrange, yrange))
    ...
    if err > errorThreshold:
        ...
        if found:
            ...
            pastHomographies[sourceFrameIndex, referenceFrameIndex] = bestHomography.astype(np.float32)
